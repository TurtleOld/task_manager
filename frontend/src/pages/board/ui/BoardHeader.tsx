...
export function BoardHeader({ boardId, boardName, onCreateColumn }: BoardHeaderProps) {
  ...
  const onArchive = async () => {
    try {
      ...
      toast.error('Не удалось архивировать доску')
    }
  }
  ...
  return (
    <header className="border-b border-border/80 bg-background/72 px-4 py-5 backdrop-blur-xl compact:py-3">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 compact:gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-3 compact:space-y-2">
          ...
          <div>
            <h1 className="mt-3 text-h1 text-text compact:mt-2">{boardName || 'Доска'}</h1>
