...
import { Button, Card as SurfaceCard, Checkbox, Chip, ChipButton, EmptyState, Field, Modal, RadioCard, Select, TextInput, Textarea } from '../../../shared/ui'
...
export function TaskModal(props: TaskModalProps) {
  ...
  return (
    <Modal
      ...
      onClose={() => { if (!saveBusy && !deleteBusy) onClose() }}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <Checkbox
                ...
                onChange={(event) => {
                  const nextEnabled = event.target.checked
                      props.reminderDrafts.forEach((item) => toggleReminder(item.id, nextEnabled))
                }}
              ...
              <div className="mt-3 space-y-3">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2 rounded-control border border-dashed border-border bg-background-subtle p-2">
                    ...
                    <Button type="button" onClick={() => addReminderInterval(newReminderValue, newReminderUnit)} disabled={!(reminderData?.deadline || draft?.deadline)} variant="secondary" size="sm">Добавить интервал</Button>
                  </div>
                ...
                {reminderDrafts.some((item) => item.status === 'invalid.past') ? <div className="rounded-control border border-warning/30 bg-warning/10 px-3 py-2 text-caption text-warning">Время напоминания уже прошло. Скорректируйте интервал или срок выполнения.</div> : null}
                {reminderDrafts.some((item) => item.status === 'invalid.channel') ? <div className="rounded-control border border-danger/30 bg-danger/10 px-3 py-2 text-caption text-danger">Канал доставки недоступен. Проверьте настройки уведомлений.</div> : null}
              </div>
            ) : null}
          </SurfaceCard>
              ...
              {selectedChecklist.length === 0 ? (
                ...
                selectedChecklist.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-3 rounded-control border border-border bg-surface px-3 py-2">
                    <Checkbox label={<span className={item.done ? 'line-through opacity-70' : ''}>{item.text}</span>} checked={item.done} onChange={() => toggleChecklistItem(item.id)} className="flex-1 border-transparent bg-transparent px-0 py-0 shadow-none" />
        ...
        <div className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Срок выполнения" htmlFor="task-deadline" hint={`Выберите дату и время завершения задачи в часовом поясе ${getTimeZoneLabel(profileTimeZone)}.`} hintId="task-deadline-hint">
              <TextInput
                id="task-deadline"
                type="datetime-local"
