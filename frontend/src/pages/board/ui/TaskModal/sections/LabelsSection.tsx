      ...
      <div className="grid gap-4">
        <div className="rounded-panel border border-border/70 bg-background-subtle/55 p-4 text-caption text-text-muted">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {allKnownLabels.length === 0 ? <span>Пока нет лейблов в этой доске.</span> : null}
          ...
          {selectedLabels.length === 0 ? (
            ...
            selectedLabels.map((label) => (
              <Chip key={label.name} style={{ borderColor: label.color, backgroundColor: `${label.color}1a`, color: label.color }}>
                {label.name}
                <button type="button" onClick={() => removeLabel(label.name)} className="text-danger hover:text-danger/80" aria-label={`Удалить лейбл ${label.name}`}>
                  ×
                </button>
              </Chip>
            ))
