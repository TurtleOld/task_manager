...
export function Modal({ children, className, footer, labelledBy, onClose, open, title }: ModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose() }}>
      <DialogContent showCloseButton={false} aria-labelledby={titleId} className={cn('w-full rounded-overlay border border-border/80 bg-[image:var(--gradient-surface)] p-6 shadow-overlay backdrop-blur', 'translate-x-[-50%] translate-y-[-50%]', 'data-[state=open]:animate-modal-content', className?.includes('max-w-') ? null : 'max-w-lg', className)}>
