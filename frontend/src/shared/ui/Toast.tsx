...
const toastTones: Record<ToastTone, string> = {
  info: 'border-info/25 bg-surface text-text',
...
export function Toast({ action, children, className, onClose, tone = 'info' }: ToastProps) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-toast w-[min(720px,calc(100%-2rem))] -translate-x-1/2">
      <div
        className={cn('rounded-overlay border px-4 py-3 shadow-overlay backdrop-blur', toastTones[tone], className)}
        role={role}
        aria-live={liveMode}
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            {action ? (
              <Button type="button" size="sm" variant="secondary" loading={action.loading} onClick={action.onClick}>
                {action.label}
              </Button>
            ) : null}
