...
- WEBPUSH_TTL_SECONDS (по умолчанию 7200) — как долго push-сервис хранит
  недоставленное сообщение. `TTL: 0` (умолчание библиотеки pywebpush) значит
  «доставить сейчас или выбросить» — телефон со спящим экраном не получает
  ничего, не позже, а вообще никогда.
- WEBPUSH_URGENCY (по умолчанию high) — `high` просит FCM разбудить Android
  из Doze немедленно; с `normal` сообщение ждёт окна обслуживания.
...
Для более глубокой диагностики — конфигурация, очередь, устройства,
отключённые настройками, последние доставки — есть отдельная команда:

```bash
cd backend
PYTHONPATH=src uv run python manage.py notifications_doctor
```

Ничего не отправляет и не меняет, только читает и печатает отчёт; код
завершения всегда 0.

### Если уведомления не приходят именно на Android

Три причины, которые не решаются кодом:

1. Отключить оптимизацию батареи для Chrome — на Samsung/Xiaomi/Huawei
   агрессивные надстройки над Doze убивают service worker раньше, чем он
   успевает обработать push, независимо от `TTL`/`Urgency` на сервере.
2. Установить PWA на домашний экран.
3. Для зеркалирования на часы — включить Chrome в списке приложений в
   приложении-компаньоне часов (Wear OS / Huawei Health); по умолчанию туда
   попадает не всё.

Отдельный случай — устройство есть в списке, `notifications_doctor` не
показывает ошибок (`failure_count=0`, `last_success_at` свежий), тестовое
уведомление тоже помечается как отправленное, но на телефон ничего не
приходит и на часы не зеркалится. Это не то же самое, что «не решается
кодом»: FCM-подписка на Android Chrome может протухнуть на стороне Google
без единого сигнала — `pushsubscriptionchange` не срабатывает, а
`fcm.googleapis.com/fcm/send/...` продолжает отвечать 201 на сообщения,
которые никогда не доходят до устройства. Определить это с сервера
невозможно: у Web Push нет квитанции о реальной доставке. Диагностируется
через `chrome://gcm-internals` на самом телефоне (запись «Data msg received»
в Receive Message Log подтверждает, что сообщение дошло до Android, дальше
уже дело SW). Чинится кнопкой **«Обновить подписку»** в настройках
уведомлений — она пересоздаёт FCM-токен на этом устройстве и заменяет
старую запись в базе. Столкнулись 22.08.2026: `Chrome на Android` слал
успешно по всем признакам, но ничего не доходило, пока подписку не
обновили вручную.

Чтобы это не повторялось молча, приложение раз в 14 дней (при открытии, не
чаще) само выполняет ту же переподписку в фоне — `maybeAutoResubscribe()` в
`frontend/src/lib/pushManager.ts`, вызывается из `AppShell`. На
`pushsubscriptionchange` не полагаемся: Chrome добавил это событие только в
137-й версии (2025), и по описанию Google оно покрывает сценарий «разрешение
отозвали и выдали заново», а не тихое протухание токена на стороне FCM —
именно то, что произошло здесь. Разрешение на уведомления при этом повторно
не запрашивается: раз оно уже выдано, `Notification.requestPermission()`
резолвится мгновенно без диалога, так что фоновая переподписка незаметна для
пользователя.
